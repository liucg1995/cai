    ...
    weChatDeposit_1:function()
    {
        ...
        function bind_event(div_id)
        {
            ...
            $("#"+div_id+" [name=next_btn]").bind("click",function(){
                ...
                var userName = $.trim($(partSlt+" [name=\"bank_cards\"]").val());
                if( "" == amount ){
                    _alert("请输入金额");
                    return;
                }
                ...
                // wechat_step1_key[arrIndex].iFlag = '1';
                // var payid =  wechat_step1_key[arrIndex].iPayID ;
                // var payid = Date.parse(new Date());
                // for (i=0;i<=6;i++){
                //     payid+=Math.ceil((Math.random()*10))*10;
                // }

                $.ajax({
                    type: 'post',
                    url: '/index.php/deposit/inRecharge',
                    dataType: 'json',
                    beforeSend:function(){
